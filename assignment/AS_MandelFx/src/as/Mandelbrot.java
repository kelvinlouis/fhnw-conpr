...
public class Mandelbrot {
	...
	public static ArrayList<Thread> computeParallel(PixelPainter painter, Plane plane, CancelSupport cancel) {
		double half = plane.length / 2;
		double reMin = plane.center.r - half;
		double imMax = plane.center.i + half;
		double step = plane.length / IMAGE_LENGTH;
		...
		for (int i = 0; i < THREADS; i++) {
			int xStart = (IMAGE_LENGTH / THREADS) * i;
			int xEnd = (IMAGE_LENGTH / THREADS) * (i+1);

			Thread t = new Thread(new RangePainter(xStart, xEnd, reMin, imMax, step, painter, cancel));
			t.start();
			ts.add(t);
		}
